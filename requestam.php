...
$con = mysqli_connect('localhost','root', '','house');//Records per page?>
   <center></center><!-- InstanceBeginEditable name="pagetitle" --><b> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</b><br></center><!-- InstanceEndEditable --></div>
      </br>
...
<tr>
<td>የተጠቃሚ  ስም  :</td><td><input name="username" type="text" size"20"  required="required" placeholder=""></input></td>
</tr>
<tr>
<td>የቤት ቁጥር   :</td><td><input name="housenumber" type="number" size"20"  title="it must be number" required Pattern="[0-9]{1,10}"></input></td>
</tr>

<tr>
      <td >መልክት አስገባ፡  : </td>
      <td bgcolor=""><label>
              <textarea colspan="2" cols="22" rows="3" name="reason" value=" " required></textarea><br>
      </label></td>
         </tr>
      <td a>ቀን፡</td>
      <td >
...
<input type="submit" value="አስገባ" name="request"/> <input type="reset" value=" አጥፋ"/>
</td></tr>
...
<?php
 ...
 $username=$_POST['username'];
         $code=$_POST['housenumber'];
  $sql="select * from account where username='{$username}' AND usertype='customer' AND status='1'";
...
$sql="insert into request1 (username,email,housenumber,reason,date)
  values('$_POST[username]','$email','$_POST[housenumber]','$_POST[reason]','$_POST[date]')";  
     $result=mysqli_query($con,$sql); 
        $result = mysqli_query($con,("update register set status='' WHERE housenumber='{$code}'")) or die(mysqli_error());  
if(!$result){
    print "<img src='IMG/error.png' align='center' width='25px' height='25px'><font face='arial' color='red' size='2'> የተሳሳተ መረጃ ሞልተዋል እባክዎ እንደገና ይሞክሩ!!</font>";
   echo' <meta content="5;request.php" http-equiv="refresh" />'; 
}

else{ 
      
  echo'  <p align="center"><font color="green" size="2"><img width="30px" height="30px" src="IMG/tick.png">&nbsp;&nbsp; በትክክል ተሳክቶልዎታል!!</font></p>';
